...
if (!extension_loaded('curl') && !ini_get('allow_url_fopen')) {
    ...
    die("❌ 当前 PHP 环境既未启用 cURL 扩展，也未开启 allow_url_fopen，无法运行本脚本。\n" .
        "请至少开启其中一项，或联系主机服务商。");
}
...
$cache_file = $cache_dir . '/list.json.cache';
$cache_time = 1800; // list.json 缓存 30 分钟（教材索引变动不频繁）

$proxies_remote_url = "https://raw.githubusercontent.com/{$github_repo}/main/proxies.json";
$proxies_cache_file = $cache_dir . '/proxies.json.cache';
$proxies_cache_time = 300; // 代理配置缓存 5 分钟

// ==========================================
// 2. 双引擎网络请求 (cURL + file_get_contents)
// ==========================================
function safe_fetch_url($url, $options = []) {
    ...
    if (extension_loaded('curl')) {
        ...
        if ($data !== false && $code >= 200 && $code < 400) {
            return ['data' => $data, 'code' => $code, 'content_type' => $content_type, 'engine' => 'curl'];
        }
        return false;
    }

    // 降级到 file_get_contents
    if (ini_get('allow_url_fopen')) {
        ...
        if ($data !== false) {
            ...
            if ($code >= 200 && $code < 400) {
                return ['data' => $data, 'code' => $code, 'content_type' => $content_type, 'engine' => 'fopen'];
            }
        }
    }
...
// ==========================================
$proxies = [];
if (file_exists($proxies_cache_file) && (time() - filemtime($proxies_cache_file)) < $proxies_cache_time) {
    $proxies_data = @file_get_contents($proxies_cache_file);
    $proxies = json_decode($proxies_data, true);
} else {
    $result = safe_fetch_url($proxies_remote_url, ['timeout' => 15]);
    if ($result !== false) {
        @file_put_contents($proxies_cache_file, $result['data']);
        $proxies = json_decode($result['data'], true);
...
function safe_encode_url($url) {
    ...
    $result = $parsed['scheme'] . '://' . $parsed['host'] . $encoded_path;
...
function get_working_proxies_sorted($proxies, $test_url) {
    ...
    foreach ($proxies as $proxy) {
        ...
        if (extension_loaded('curl')) {
            ...
            // 尝试 2: 如果 HEAD 返回 405，用 Range GET
            if (!$success && $code === 405) {
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $target);
            ...
            // 无 cURL 时，用 file_get_contents Range GET 测速
            $result = safe_fetch_url($target, ['timeout' => 3, 'range' => '0-0']);
            if ($result !== false && $result['code'] >= 200 && $result['code'] < 400) {
                $success = true;
                $total_time = 1.0; // 无法精确计时，给个中间值
            }
        }

        if ($success) {
            $working_nodes[] = ['proxy' => $proxy, 'time' => $total_time];
        }
    }
...
// 更严格的路径过滤：去除 .. 、反斜杠、空字节，以及 URL 编码后的变体
$requested_file = urldecode($requested_file);
$requested_file = str_replace(['..', '\\', "\0", '%00'], '', $requested_file);

// ==========================================
// 7. list.json 缓存读取与数据清洗
// ==========================================
$json_data = false;
if (file_exists($cache_file) && (time() - filemtime($cache_file)) < $cache_time) {
    $json_data = @file_get_contents($cache_file);
} else {
    $result = safe_fetch_url($list_json_raw_url, ['timeout' => 20]);
    if ($result !== false) {
        ...
        foreach ($cdn_proxies as $proxy) {
            ...
            $result = safe_fetch_url($pull_url, ['timeout' => 20]);
            if ($result !== false) {
                $json_data = $result['data'];
                break;
            }
...
$files = array_filter($files, function($file) {
    return isset($file['path']) && strpos($file['path'], '.cache') !== 0;
});
...
if (empty($requested_file)) {
    ...
    <head>
        ...
        <title>中小学课本直通车 - 高清电子教科书免费下载</title>
        <meta name="keywords" content="中小学课本,电子教科书,电子版教材,电子书下载,人教版电子课本,统编版教材,高清PDF教材">
        ...
        <style>
            ...
            @media (max-width: 600px) {
                ...
                .download-btn { position: absolute; right: 16px; bottom: 16px; padding: 6px 14px; font-size: 0.85rem; margin: 0; border-radius: 8px; z-index: 2; } 
            ...
            function renderFiles() {
                ...
                Array.from(folders).sort().forEach(folder => {
                    ...
                    div.innerHTML = `
                        ...
                        <div class="info folder-info"><div class="name">${folder}</div></div>
...
function stream_file_with_curl($url, $filename) {
    ...
    curl_setopt($ch, CURLOPT_TIMEOUT, 0); // 无限制，大文件需要
...
// 10. 下载分发逻辑
// ==========================================
if (isset($target_file_data['is_chunked']) && $target_file_data['is_chunked'] && isset($target_file_data['chunks'])) {
    // 分片文件：服务器端流式合并
    ...
    // 普通文件：302 重定向，但先做 Range 预检确保不是 HTML 拦截页
    ...
    foreach ($working_proxies as $proxy) {
        ...
        // 用 Range 请求预检：确保返回的是二进制文件，而非 HTML 错误页
        $result = safe_fetch_url($file_download_url, ['range' => '0-1023', 'timeout' => 5]);

        if ($result !== false && $result['code'] >= 200 && $result['code'] < 400) {
            $ct = strtolower($result['content_type'] ?? '');
            $data = $result['data'] ?? '';

            // 检查是否为 HTML 拦截页（某些代理被封时返回 HTML）
            $is_html = (strpos($ct, 'text/html') !== false) || 
                       (stripos($data, '<html') !== false) ||
                       (stripos($data, '<!doctype') !== false);
