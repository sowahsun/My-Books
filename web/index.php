...
if (file_exists($cache_file) && (time() - filemtime($cache_file)) < $cache_time) {
    ...
    $json_data = file_get_contents($list_json_raw_url, false, $stream_context);
    if ($json_data === false) {
        ...
        foreach ($cdn_proxies as $proxy) {
            ...
            $json_data = file_get_contents($pull_url, false, $stream_context);
            if ($json_data !== false) {
                break;
            }
        }
...
if ($target_file_data['is_chunked'] && isset($target_file_data['chunks'])) {
    foreach ($target_file_data['chunks'] as $chunk) {
        ...
        foreach ($working_proxies as $proxy) {
            ...
            $handle = @fopen($chunk_download_url, 'rb', false, $stream_context);
    ...
    foreach ($working_proxies as $proxy) {
        ...
        $handle = @fopen($file_download_url, 'rb', false, $stream_context);
